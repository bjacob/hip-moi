...
for mask in "$@"; do
  exe="${BUILD_DIR}/attention_${mask}"
  fatbin="${BUILD_DIR}/attention_${mask}.fatbin"
  device="${BUILD_DIR}/attention_${mask}.device.o"
  ...
  "${HIPCC}" \
    ...
    "${SCRIPT_DIR}/010_rdna4_wmma_attention_block_benchmark.hip" \
    -o "${exe}"
  ...
  "${OBJDUMP}" -d -C --mcpu="${OFFLOAD_ARCH}" "${device}" | awk '
    ...
    /^0[0-9a-f]+ </ {
      ...
      name = "";
      if($0 ~ /attention_block_kernel</) {
        name = $0;
        sub(/^.*attention_block_kernel</, "", name);
        sub(/Policy>.*$/, "", name);
