...
if [[ -n "${ROCM_ROOT:-}" ]]; then
  ...
  echo "error: expected TheRock ROCm SDK in ${VENV} or ROCM_ROOT=/path/to/rocm" >&2
  exit 1
fi
...
OUT="${BUILD_DIR}/pingpong_att_probe"
...
"${HIPCC}" \
  ...
  "${SCRIPT_DIR}/016_rdna4_pingpong_att_probe.hip" \
  -o "${OUT}"

if [[ "${HIP_MOI_BUILD_ONLY:-0}" != "0" ]]; then
  echo "${OUT}"
  exit 0
fi
