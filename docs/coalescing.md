...
when an opted-in context site is summarized for an epoch-close conflict
check.

Coalescing is a context optimization layered on top of that exact model.
...
For context summaries and hot-path compression, a nonzero site id is
necessary but not sufficient: the context also needs coalescing
access storage. `hip_moi::host_context` can provide that storage
...
storage to make context summary construction cheaper across many
...
coalescing metadata is produced. For context, users may also leave
...
each opted-in context access that takes this exact-record fallback path.
Default-site accesses do not increment that counter.

### Opted-In Sites

In context, an opted-in access writes a
...
counter is present. The context diagnostic `access_record` stays compact
...
Today, context opt-in therefore costs:
...
* optionally, one context group scratch record per distinct
...
For context opted-in accesses, coalescing now reduces ordinary exact
...
The remaining known context cost is the validation scan per candidate
...
`coalescing_access_count` counts opted-in context accesses that entered
...
`coalescing_fallback_count` counts opted-in context accesses that could
...
`coalescing_group_count` counts context group scratch slots occupied at
...
Context summaries use
`context::coalesced_access_record`. Conceptually, a summary says:
...
Context conflict detection has two layers:
...
When a context diagnostic is emitted from a coalesced summary, the
...
Because context checking is epoch-close based, kernels whose final epoch
...
It only runs in context at `ctx.syncthreads()` or a final uniform
`ctx.finish()`. The context proves patterns from the separate
...
* a single summary spanning multiple subgroups; context emits one
  summary per subgroup,
* context opted-in accesses when no coalescing access storage was
...
The accelerated context grouped path is not used when no group scratch
...
simple, regular all-lane LDS access sites in context. It has started
reducing exact access-log traffic for opted-in context sites, but it is
